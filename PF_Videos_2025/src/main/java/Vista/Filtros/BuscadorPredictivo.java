...
public class BuscadorPredictivo {
    ...
    private static List<String> obtenerSugerenciasDesdeBD(String consulta) {
        List<String> mockBD = new ArrayList<>();
        mockBD.add("Ariel la Sirenita - ID: 101");
        mockBD.add("Ariel 2: Regreso al Mar - ID: 102");
        mockBD.add("Avatar - ID: 204");
        
        List<String> filtradas = new ArrayList<>();
        for (String item : mockBD) {
            if (item.toLowerCase().contains(consulta.toLowerCase())) {
                filtradas.add(item);
            }
        }
        return filtradas;
